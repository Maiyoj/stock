...
// approval begins here
 ...
 #dd($items);
   ...
   $comment->save();
    ...
    foreach($items as  $item)
    {
     $team_lead=TeamLeadStock::where('teamlead_id',$request->user_id)->where('teamlead_id',$item->item_id)->first();
     if( $team_lead==null)  
...
*/
      ...
      Session::forget('request');
      return redirect()->route('request.drafts')->with('success','Draft  Created successfully');
   }
